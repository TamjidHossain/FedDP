...
with open('D:/household_power_consumption/boston_housing.pickle','rb') as f:
...
class server():
    def __init__(self, train, test, train_len, test_len, number_clients, p_budget, epsilon, sigmat = 1.12):
        ...
        for i in range(number_clients):
            # loader = torch.utils.data.DataLoader(mnist_trainset, batch_size=13, sampler=self.samples[i])
            loader = DataLoader(train, batch_size=13, sampler=self.samples[i])
            self.clients.append(client(i, loader, self.model.state_dict()))
        self.p_budget = p_budget
        self.epsilon = epsilon
        self.testLoader = torch.utils.data.DataLoader(test, batch_size=13)
...
class client():
    def __init__(self, number, loader, state_dict, batch_size = 13, epochs=10, lr=0.001):
        self.number = number
        self.model = t_model()
